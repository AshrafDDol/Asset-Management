...
const WorkQueueScreen: React.FC<Props> = ({ navigation }) => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      // The navigation API requires a render callback for headerRight.
      // eslint-disable-next-line react/no-unstable-nested-components
      headerRight: () => (
        <SettingsButton
          onPress={() => navigation.navigate('RFIDSettingsScreen')}
        />
      ),
    });
  }, [navigation]);
...
const styles = StyleSheet.create({
  ...
  settingsButton: { borderWidth: 1, borderColor: '#CBD5E1', borderRadius: 16, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: '#FFFFFF' }, settingsText: { color: '#1D4ED8', fontWeight: '700' },
});
