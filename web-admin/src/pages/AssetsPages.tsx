...
function PrepareIssueDialog({
  ...
  return (
    <dialog
      ref={ref}
      className="asset-prepare-dialog"
      onCancel={(e) => {
        e.preventDefault();
        if (!busy) close();
      }}
    >
      <div className="page-title-row">
        <div>
          <h3>Prepare Issue Batch</h3>
          <p>{assets.length} exact physical Assets selected</p>
        </div>
      ...
      <form onSubmit={submit}>
        <div className="form-grid">
          <label className="form-field job-combobox">
            Job No. *
            <input
              required
              role="combobox"
            ...
            {!selectedBatch &&
              jobNo.trim() &&
              (jobMatches.length ? (
                <div className="job-combobox-results">
                  {jobMatches.map((batch) => {
                    ...
                    return (
                      ...
                      >
                        <strong>{batch.jobNo}</strong>
                        <span>
                          {batchState(batch)} · {batch.batchNo}
                        </span>
                        <small>
                          {counts.awaiting} Awaiting · {counts.inUse} In Use ·{" "}
          ...
          <label className="form-field">
            ...
            <select
              value={recipient}
              onChange={(e) => apply("recipient", e.target.value)}
            >
        ...
        {selectedBatch && (
          <div className="job-selected-summary">
            <strong>Existing Job · {selectedBatch.jobNo}</strong>
            <span>
              Issue Batch: {selectedBatch.batchNo} · Status:{" "}
              {batchState(selectedBatch)}
            </span>
            <span>
              Awaiting Confirmation: {batchCounts(selectedBatch).awaiting} · In
              Use: {batchCounts(selectedBatch).inUse} · Returned:{" "}
              {batchCounts(selectedBatch).returned}
            </span>
          </div>
        )}
            ...
            <tbody>
              {assets.map((asset) => (
                <tr key={asset.id}>
                  ...
                  <td>
                    {asset.measurementHeight ?? "-"} ×{" "}
                    {asset.measurementWidth ?? "-"}
                  </td>
                  <td>{asset.epc?.epcCode}</td>
                  <td>{locationDisplayName(asset.location)}</td>
                  <td>
                    <select
        ...
        <div className="form-actions">
          ...
          {selectedBatch && assets.length !== 1 && (
            <span>Swap Asset requires exactly one replacement Asset.</span>
          )}
        </div>
      </form>
      {swapOpen && selectedBatch && assets.length === 1 && (
        <div className="modal-backdrop">
          <div className="modal-card swap-asset-modal">
            <div className="page-title-row">
              <div>
                <h3>Swap Asset - {selectedBatch.jobNo}</h3>
                <p>
                  The selected Asset is the replacement and will await normal
                  EPC confirmation.
                </p>
              </div>
              <button
            ...
            <section className="swap-replacement-summary">
              <strong>Replacement Asset</strong>
              <span>
                {assets[0].assetCode} · {assets[0].itemName} /{" "}
                {assets[0].category?.name || "—"}
              </span>
              <span>
                {assets[0].measurementHeight ?? "—"} ×{" "}
                {assets[0].measurementWidth ?? "—"} mm · EPC:{" "}
                {assets[0].epc?.epcCode || "—"}
              </span>
              <span>
            ...
            {swapTarget ? (
              ...
              >
                <h4>{swapTarget.status === "ISSUED" ? "Replace Pending Selection" : "Prepare Handheld Swap"}</h4>
                <p>
                  Existing: <strong>{swapTarget.asset.assetCode}</strong>
                </p>
                {swapTarget.status === "CONFIRMED" && (
                  <section className="swap-replacement-summary">
                    <strong>Existing Asset</strong>
                    <span>{swapTarget.asset.assetCode}</span>
                    <span>
                      EPC Verification: {swapOldVerified ? "✓ Old EPC Verified" : "Waiting"}
                    </span>
                    {!swapOldVerified && (
                      <label className="form-field">
                        Scan / Enter Old EPC
                    ...
                    {swapVerificationError && (
                      <span className="error-box">{swapVerificationError}</span>
                    )}
                    {!swapOldVerified && (
                      <button
                        type="button"
                ...
                <div className="form-actions">
                  ...
                  <button className="primary-button" disabled={busy}>
                    {busy ? "Saving..." : swapTarget.status === "ISSUED" ? "Replace Selection" : "Create Handheld Swap"}
                  </button>
                </div>
              </form>
            ) : (
              <>
                <h4>Select Asset to Replace</h4>
                {swapCandidates.length === 0 ? (
                  <p>
                    No awaiting-confirmation or in-use Assets are eligible in
                    this Job.
                  </p>
                ) : (
                      ...
                      <tbody>
                        {swapCandidates.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td>
                              {item.asset.itemName}
                              <small>{item.asset.category?.name || "—"}</small>
                            </td>
                            <td>
                              {item.asset.measurementHeight ?? "—"} ×{" "}
                              {item.asset.measurementWidth ?? "—"} mm
                            </td>
                            <td>{item.asset.epc?.epcCode || "—"}</td>
                            <td>{locationDisplayName(item.asset.location)}</td>
                            <td>
                            ...
                            <td>
                              ...
                              {item.handheldSwapTasks?.[0] && (
                                <small>
                                  Existing Asset {item.handheldSwapTasks[0].oldVerifiedEpc ? "✓ Verified" : "Waiting"}
                                  <br />
                                  Replacement Asset {item.handheldSwapTasks[0].newVerifiedEpc ? "✓ Verified" : "Waiting"}
                                </small>
                              )}
                            </td>
                            <td>
...
function AssetEpcDialog({
  ...
  return (
    <dialog
      ref={ref}
      className="asset-epc-dialog"
      aria-label={replacing ? "Replace EPC" : "Assign EPC"}
      onCancel={(event) => {
        event.preventDefault();
        if (!busy) close();
      }}
    >
      <div className="page-title-row">
        <div>
          <h3>{replacing ? "Replace EPC" : "Assign EPC"}</h3>
          <p>
      ...
      <div className="epc-current">
        <strong>Current EPC</strong>
        <span>{replacing ? asset.epc?.epcCode : "Not Assigned"}</span>
        <small>Status: {replacing ? "Active" : "Not Assigned"}</small>
      </div>
      {error && (
      ...
      {savedEpc && (
        <div className="success-box" role="status">
          EPC saved successfully: <strong>{savedEpc}</strong>
        </div>
      )}
      <form onSubmit={submit}>
        ...
        <label className="form-field">
          {autoGenerate ? "EPC (generated by server)" : "Manual EPC Entry *"}
          <input
        ...
        <div className="form-actions">
          <button className="primary-button" disabled={busy || !!savedEpc}>
            {busy
              ? "Saving..."
              : savedEpc
              ? "Saved"
  ...
  const renderAssetCell = (asset: Asset, column: AssetColumnId) => {
    switch (column) {
      ...
      case "epc":
        return (
          <>
            {hasActiveEpc(asset) ? asset.epc?.epcCode : "Not Assigned"}
            <small
              className={hasActiveEpc(asset) ? "epc-active" : "epc-missing"}
            >
              {hasActiveEpc(asset) ? "Active" : "Not Assigned"}
            </small>
          </>
        );
  ...
  return (
    <div className="page">
      ...
      <div className="form-panel asset-search-panel">
        ...
        <fieldset className="asset-detail-filter-group">
          <legend>Measurement / Blade Details</legend>
          <div className="asset-filter-row">
            <label className="form-field">
              Height (mm)
              <input
        ...
        <div className="asset-filter-row">
          ...
          <label className="form-field">
            ...
            >
              <option value="">All</option>
              {["AVAILABLE", "RESERVED", "PENDING_CONFIRMATION", "IN_USE"].map(
                (v) => (
                  <option key={v}>{v}</option>
                )
              )}
          ...
          <label className="form-field">
            ...
            <select
              value={order}
              onChange={(event) => setOrder(event.target.value as ListOrder)}
            >
      ...
      <div className="asset-selection-tray">
          ...
          onClick={() =>
            setSelected(
              (old) =>
                new Set([
                  ...old,
                  ...visible.filter((a) => !eligibility(a)).map((a) => a.id),
                ])
            )
          }
        >
      ...
      {assetFormOpen && (
        <div className="modal-backdrop" role="presentation">
          <div
            className="asset-registration-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="page-title-row">
              <div>
                <h3 id="asset-form-title">
                  {isEditing ? "Edit Asset" : "Register Asset"}
                </h3>
                <p>
                  {isEditing
            ...
            {error && <div className="error-box">{error}</div>}
            <form
              className="form-panel asset-registration-form"
              onSubmit={async (event) => {
                if (await handleSubmitAsset(event)) setAssetFormOpen(false);
              }}
            >
              <div className="form-grid">
                <h4 className="asset-form-section-title">
                  1. Basic Information
                </h4>
                ...
                <h4 className="asset-form-section-title">
                  2. Measurement &amp; Asset Details
                </h4>
                <div className="form-field">
                  <label>Measurement Height (mm)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    onChange={(event) =>
                      updateForm("measurementHeight", event.target.value)
                    }
                  />
                </div>
                <div className="form-field">
                  <label>Measurement Width (mm)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    onChange={(event) =>
                      updateForm("measurementWidth", event.target.value)
                    }
                  />
                </div>
                <div className="form-field">
                  ...
                  <input
                    ...
                    value={form.gridUp}
                    onChange={(event) =>
                      updateForm("gridUp", event.target.value)
                    }
                  />
                </div>
                <div className="form-field">
                ...
                <div className="form-field">
                  ...
                  <input
                    ...
                    value={form.gapMm}
                    onChange={(event) =>
                      updateForm("gapMm", event.target.value)
                    }
                  />
                </div>
                ...
                {isEditing && locationLocked && (
                  <div className="form-field">
                    ...
                    <LocationTreeSelect
                      ...
                      onChange={(id) =>
                        updateForm("homeLocationId", String(id))
                      }
                      allowedLocation={storageLocation}
                      placeholder={
                        homeNeedsVerification
                          ? "Select verified storage home"
                          : "Verified"
                      }
                    />
                    {homeNeedsVerification && (
                      <small>
                        Initializes the missing legacy home only; current
                        verified location will not change.
                      </small>
                    )}
                  </div>
                )}
                    ...
                    <div className="form-field checkbox-field">
                      <label className="checkbox-row">
                        <input
                          ...
                          onChange={(event) =>
                            updateForm("registerEpc", event.target.checked)
                          }
                        />
                        Register EPC
                      </label>
                        ...
                        <div className="form-field">
                          ...
                          <label className="checkbox-row">
                            <input
                              ...
                              onChange={() =>
                                updateForm("autoGenerateEpc", true)
                              }
                            />
                            Auto Generate
                          </label>
                          <label className="checkbox-row">
                            <input
                              ...
                              checked={!form.autoGenerateEpc}
                              onChange={() =>
                                updateForm("autoGenerateEpc", false)
                              }
                            />
                            Manual
                          </label>
                ...
                {isEditing && editingAsset && (
                  <div className="form-field asset-edit-epc">
                    <label>Current EPC</label>
                    <strong>
                      {hasActiveEpc(editingAsset)
                        ? editingAsset.epc?.epcCode
                        : "Not Assigned"}
                    </strong>
                    <small>
                      Status:{" "}
      ...
      <div className="asset-table-toolbar">
        ...
        {columnSelectorOpen && (
          <AssetColumnSelector
            columns={columns}
            close={() => setColumnSelectorOpen(false)}
            apply={applyColumns}
          />
        )}
      </div>
          ...
          <table className="data-table">
            <thead>
              <tr>
                <th>Select</th>
                {columns.map((column) => (
                  <th key={column}>{assetColumnLabel.get(column)}</th>
                ))}
              </tr>
            </thead>
                  ...
                  <tr key={asset.id}>
                    <td>
                      <input
                        ...
                        title={
                          eligibility(asset) || "Eligible for Issue preparation"
                        }
                        onChange={(e) =>
                          setSelected((old) => {
                    ...
                    {columns.map((column) => (
                      <td key={column}>{renderAssetCell(asset, column)}</td>
                    ))}
                  </tr>
                ))
              )}
          ...
          complete={async (count, message) => {
            ...
            navigate("/issue-batches", {
              state: {
                message:
                  message ||
                  `${count} Assets prepared. Awaiting EPC confirmation.`,
              },
            });
          }}
        />
