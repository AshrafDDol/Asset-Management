...
export function AssetsPages() {
  ...
  return (
    <div className="page">
      ...
      <div className="form-panel asset-search-panel"><strong>Operational Asset Search</strong><div className="form-grid"><label className="form-field">Asset Code<input value={filters.assetCode} onChange={(e) => setFilter("assetCode", e.target.value)} /></label><label className="form-field">Item Name<input value={filters.itemName} onChange={(e) => setFilter("itemName", e.target.value)} /></label><label className="form-field">Category<select value={filters.category} onChange={(e) => setFilter("category", e.target.value)}><option value="">All</option>{assetCategories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label className="form-field">Measurement H<input type="number" value={filters.height} onChange={(e) => setFilter("height", e.target.value)} /></label><label className="form-field">Measurement W<input type="number" value={filters.width} onChange={(e) => setFilter("width", e.target.value)} /></label><label className="form-field">EPC<input value={filters.epc} onChange={(e) => setFilter("epc", e.target.value)} /></label><label className="form-field">Current Location<LocationTreeSelect locations={locations} selectedLocationId={filters.current} onChange={(id) => setFilter("current", String(id))} allowClear clearLabel="All Locations" onClear={() => setFilter("current", "")} placeholder="All Locations" /></label><label className="form-field">Condition<select value={filters.condition} onChange={(e) => setFilter("condition", e.target.value)}><option value="">All</option>{["NEW", "GOOD", "FAIR", "POOR", "DAMAGED"].map((v) => <option key={v}>{v}</option>)}</select></label><label className="form-field">Status<select value={filters.status} onChange={(e) => setFilter("status", e.target.value)}><option value="">All</option>{["AVAILABLE", "RESERVED", "PENDING_CONFIRMATION", "IN_USE"].map((v) => <option key={v}>{v}</option>)}</select></label><label className="form-field">Order<select value={order} onChange={(event) => setOrder(event.target.value as ListOrder)}><option value="LATEST">Latest</option><option value="OLDEST">Oldest</option></select></label></div></div>
      ...
      <form className="form-panel asset-registration-form" onSubmit={async (event) => { if (await handleSubmitAsset(event)) setAssetFormOpen(false); }}>
        <div className="form-grid">
          ...
          {!isEditing && <>
            ...
            <div className="form-field"><label>EPC {form.autoGenerateEpc ? "(Generated by server)" : "(Manual) *"}</label><input disabled={form.autoGenerateEpc} required={!form.autoGenerateEpc} minLength={24} maxLength={24} pattern="[0-9A-Fa-f]{24}" value={form.epcCode} onChange={(event) => updateForm("epcCode", event.target.value)} placeholder={form.autoGenerateEpc ? "Generated after registration" : "Paste or enter 24 hexadecimal characters"} /></div>
          </>}

          {isEditing && editingAsset && <div className="form-field asset-edit-epc"><label>EPC</label><strong>{hasActiveEpc(editingAsset) ? editingAsset.epc?.epcCode : "Not Assigned"}</strong><small>Status: {hasActiveEpc(editingAsset) ? "Active" : "Not Assigned"}</small><button type="button" className="secondary-button" disabled={!epcMutable(editingAsset)} title={epcMutable(editingAsset) ? undefined : "EPC changes are allowed only while Available or Reserved"} onClick={() => setEpcAsset(editingAsset)}>{hasActiveEpc(editingAsset) ? "Replace EPC" : "Assign EPC"}</button></div>}
          ...
          <table className="data-table">
            ...
            <tbody>
              {visible.length === 0 ? (
                ...
                visible.map((asset) => (
                  <tr key={asset.id}>
                    <td><input type="checkbox" checked={selected.has(asset.id)} disabled={!!eligibility(asset)} title={eligibility(asset) || "Eligible for Issue preparation"} onChange={(e) => setSelected((old) => { const next = new Set(old); if (e.target.checked) next.add(asset.id); else next.delete(asset.id); return next; })} />{eligibility(asset) && <small className="epc-missing">{eligibility(asset)}</small>}</td>
                    ...
                    <td>{hasActiveEpc(asset) ? asset.epc?.epcCode : "Not Assigned"}<small className={hasActiveEpc(asset) ? "epc-active" : "epc-missing"}>{hasActiveEpc(asset) ? "Active" : "Not Assigned"}</small></td>
                    ...
                    <td>
                      <button
                        ...
                        type="button"
                        onClick={() => { handleEditAsset(asset); setAssetFormOpen(true); }}
                      >
                        Edit
                      </button>
                    </td>
