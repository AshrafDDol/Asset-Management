...
export function IssueBatchesPages() {
  ...
  return (
    <div className="page issue-batch-workspace">
      <div className="page-title-row">
        <div>
          <h2>Issue Batches</h2>
          <p>Verify prepared Assets before physical issue and return.</p>
        </div>
        <div className="form-actions">
          <label className="compact-order-control">
            Order
            <select
              value={order}
      ...
      <div className="issue-tabs">
        ...
        >
          Issue &amp; Confirmation
          <span
            className="issue-tab-progress"
            title={`${allJobsAwaitingCount} Assets awaiting EPC confirmation across open Jobs`}
          >
            {allJobsAwaitingCount}
          </span>
        </button>
        <button
        ...
        >
          ...
          <span
            className="issue-tab-progress"
            title={`${returningCount} Assets currently returnable across open Jobs`}
          >
            {returningCount}
          </span>
        </button>
      ...
      {tab === "ISSUE" && (
        <div className="issue-global-actions">
          <button
          ...
          <span>{allJobsAwaitingCount} Awaiting EPC Confirmation</span>
        </div>
      )}
      {tab === "ISSUE" &&
        (activeIssueBatches.length === 0 ? (
          ...
          activeIssueBatches.map((batch) => {
            ...
            const awaitingSwap = items.filter(
              (item) => !!item.handheldSwapTasks?.length
            );
            const confirmed = items.filter(
              (item) => item.status === "CONFIRMED"
            );
            return (
              <section className="issue-batch-card" key={batch.id}>
                <header className="issue-batch-header">
                  <div>
                    <h3>{batch.batchNo}</h3>
                    {batch.jobNo && <p>Job No: {batch.jobNo}</p>}
                    <strong>{awaiting.length} Awaiting Confirmation</strong>
                    {awaitingSwap.length > 0 && (
                      <span> · {awaitingSwap.length} Awaiting Swap</span>
                    )}
                    {confirmed.length > 0 && (
                      <span> · {confirmed.length} In Use</span>
                    )}
                  </div>
                  <div className="form-actions">
                    <button
                    ...
                    <tbody>
                      {items.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>{item.asset.epc?.epcCode || "Missing"}</td>
                          <td>
      ...
      {tab === "RETURNING" && (
        <section className="table-panel">
          <div className="page-title-row">
            <div>
              <h3>Assets Currently In Use</h3>
              <p>
                Scan returned EPCs from any Issue Batch. Verified Assets return
                directly to their registered home location.
              </p>
            </div>
            <button
              className="primary-button"
          ...
          <div className="issue-batch-workspace">
            {returnJobs.map(({ batch, items }) => (
              <section className="issue-batch-card" key={`return-${batch.id}`}>
                <header className="issue-batch-header">
                  <div>
                    <h3>{batch.jobNo || batch.batchNo}</h3>
                    <strong>Returning {items.length}</strong>
                    <span> · {items.length} asset{items.length === 1 ? "" : "s"} still IN_USE</span>
                  </div>
                </header>
              </section>
            ))}
            ...
            <table className="data-table">
              ...
              <tbody>
                {returningItems.length === 0 ? (
                  <tr>
                    <td colSpan={7}>
                      No confirmed Assets are currently in use.
                    </td>
                  </tr>
                ) : (
                  returningItems.map(({ batch, item }) => (
                    <tr key={item.id}>
                      <td>
                        {batch.batchNo}
                        <small>{batch.jobNo || "No Job No."}</small>
                      </td>
                      <td>
                        {item.asset.assetCode}
                        <small>{item.asset.itemName}</small>
                      </td>
                      <td>
                        {item.recipient?.fullName ||
                          item.recipient?.username ||
      ...
      {swapVerificationTask && (
        <div className="modal-backdrop">
          <form className="modal-card" onSubmit={submitSwapVerification}>
            <div className="page-title-row">
              <div>
                <h3>Swap EPC Verification</h3>
                <p>
            ...
            <div className="job-selected-summary">
              <strong>
                Existing Asset {swapOldVerified ? "✓ Verified" : "Waiting"}
              </strong>
              <span>{swapVerificationTask.targetItem.asset.assetCode}</span>
              <strong>
                Replacement Asset {swapNewVerified ? "✓ Verified" : "Waiting"}
              </strong>
              <span>{swapVerificationTask.replacementAsset.assetCode}</span>
            </div>
            {!swapNewVerified && (
              <label className="form-field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setSwapVerificationText(event.target.value)
                  }
                />
              </label>
            )}
            <div className="form-actions">
              ...
              {swapOldVerified && swapNewVerified && (
                <button
                  ...
                  className="primary-button"
                  disabled={busy}
                  onClick={() => void confirmVerifiedSwap()}
                >
      ...
      {scanBatch && (
        <div className="modal-backdrop">
          <form className="modal-card" onSubmit={submitScan}>
            <div className="page-title-row">
              <div>
                <h3>{scanItem ? "Confirm EPC" : "Scan All"}</h3>
                <p>
                  {scanItem
            ...
            <label className="form-field">
              {scanItem ? "Paste / Enter EPC" : "Paste EPCs"}
              {scanItem ? (
                <input
                  autoFocus
                  required
            ...
            <button className="primary-button" disabled={busy}>
              {busy
                ...
                : scanItem
                ? "Confirm EPC"
                : "Compare and Confirm Matches"}
            </button>
            {scanResults.length > 0 && (
              <ul>
                {scanResults.map((result, index) => (
                  <li key={`${result.epc}-${index}`}>
                    <code>{result.epc || "(blank)"}</code> —{" "}
                    {result.classification.toLowerCase().replaceAll("_", " ")}
                  </li>
                ))}
              </ul>
            )}
      ...
      {returnScanOpen && (
        <div className="modal-backdrop">
          <form
            className="modal-card return-scan-modal"
            onSubmit={submitReturnScan}
          >
            <div className="page-title-row">
              <div>
                <h3>Return Scan</h3>
                <p>
                  Paste EPCs from one or more Issue Batches. Each EPC is
                  validated and returned independently.
                </p>
              </div>
              <button
                type="button"
                className="secondary-button"
                onClick={() => setReturnScanOpen(false)}
              >
                Close
              </button>
            </div>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {returnResults.map((result, index) => (
                      <tr key={`${result.epc}-${index}`}>
                        <td>{result.assetCode || "—"}</td>
                        <td>
                          <code>{result.epc || "(blank)"}</code>
                        </td>
                        <td>{result.previousLocation || "—"}</td>
      ...
      {scanAllJobsOpen && (
        <div className="modal-backdrop">
          <form
            className="modal-card scan-all-jobs-modal"
            onSubmit={submitAllJobsScan}
          >
            <div className="page-title-row">
              <div>
                <h3>Scan All Jobs</h3>
                <p>
                  Paste EPCs from any active Issue Batch. Expected Assets are
                  matched to their prepared Job.
                </p>
              </div>
              <button
                type="button"
                className="secondary-button"
                onClick={() => setScanAllJobsOpen(false)}
              >
                Close
              </button>
            </div>
                  ...
                  <tbody>
                    {allJobsResults.map((result, index) => (
                      <tr key={`${result.epc}-${index}`}>
                        ...
                        <td>{scanResultLabel(result.classification)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
